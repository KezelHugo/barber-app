...
export default function AdminUsersScreen() {
  ...
  useEffect(() => {
    ...
    const isEditing = selectedAdmin !== null;
    ...
    // 1. Name unique check
    if (nameLower) {
      const nameDuplicate = admins.some(a => a.name.toLowerCase() === nameLower && (!isEditing || a.id !== selectedAdmin.id));
      if (nameDuplicate) {
        setNameError('Ya existe un administrador registrado con este nombre.');
      } else {
        setNameError('');
      }
    } else {
    ...
    if (phoneTrim) {
      if (!/^\d{9}$/.test(phoneTrim)) {
        ...
        const phoneDuplicate = admins.some(a => a.phone === phoneTrim && (!isEditing || a.id !== selectedAdmin.id));
        if (phoneDuplicate) {
          setPhoneError('Ya existe un administrador registrado con este teléfono.');
        } else {
          setPhoneError('');
        }
      }
    ...
    if (emailLower) {
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailLower)) {
        ...
        const emailDuplicate = admins.some(a => a.email.toLowerCase() === emailLower && (!isEditing || a.id !== selectedAdmin.id));
        if (emailDuplicate) {
          setEmailError('Ya existe un administrador registrado con este correo.');
        } else {
          setEmailError('');
        }
      }
    ...
    if (dniTrim) {
      if (!/^\d{8}$/.test(dniTrim)) {
        ...
        const dniDuplicate = admins.some(a => a.dni === dniTrim && (!isEditing || a.id !== selectedAdmin.id));
        if (dniDuplicate) {
          setDniError('Ya existe un administrador registrado con este DNI.');
        } else {
          setDniError('');
        }
      }
  ...
  }, [name, phone, email, dni, admins, selectedAdmin, formVisible, loadingSave]);

  // Snackbar states
  const [snackbarVisible, setSnackbarVisible] = useState(false);
  const [snackbarMsg, setSnackbarMsg] = useState('');

  // Listen to Firestore collection in real-time (querying 'users' where role is 'admin')
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error al suscribirse a admins en users:", err);
      setLoading(false);
    });
  ...
  const handleSave = async () => {
    ...
    const nameDuplicate = admins.some(a => a.name.toLowerCase() === nameLower && (!isEditing || a.id !== selectedAdmin.id));
    if (nameDuplicate) {
      setError('Ya existe un administrador registrado con este nombre.');
      return;
    }
    const phoneDuplicate = admins.some(a => a.phone === phoneTrim && (!isEditing || a.id !== selectedAdmin.id));
    if (phoneDuplicate) {
      setError('Ya existe un administrador registrado con este teléfono.');
      return;
    }
    const emailDuplicate = admins.some(a => a.email.toLowerCase() === emailLower && (!isEditing || a.id !== selectedAdmin.id));
    if (emailDuplicate) {
      setError('Ya existe un administrador registrado con este correo electrónico.');
      return;
    }
    const dniDuplicate = admins.some(a => a.dni === dniTrim && (!isEditing || a.id !== selectedAdmin.id));
    if (dniDuplicate) {
      setError('Ya existe un administrador registrado con este DNI.');
      return;
    }
    ...
    try {
      ...
      if (selectedAdmin) {
        ...
        const docRef = doc(db, 'users', selectedAdmin.id);
        await updateDoc(docRef, adminData);
        setSnackbarMsg(`Administrador "${name}" actualizado con éxito.`);
      } else {
        // Create user in Firebase Auth using a secondary app instance
        const secondaryApp = initializeApp(firebaseConfig, "SecondaryAppAdmins");
        ...
        try {
          ...
          setSnackbarMsg(`Administrador "${name}" registrado con éxito.`);
        } catch (authErr: any) {
          console.error("Error al registrar administrador en Firebase Auth:", authErr);
          if (authErr.code === 'auth/email-already-in-use') {
            ...
            setError('Error al registrar las credenciales del administrador.');
          }
      ...
      console.error("Error al guardar administrador en Firestore:", err);
  ...
  const handleDelete = async () => {
    if (selectedAdmin) {
      ...
      try {
        ...
        setSnackbarMsg(`Administrador "${selectedAdmin.name}" eliminado.`);
        setSnackbarVisible(true);
      } catch (err) {
        console.error("Error al eliminar administrador en Firestore:", err);
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface }]}>
        ...
        <Text variant="bodySmall" style={styles.headerSubtitle}>
          Administra los roles de gestión, datos de DNI y control de accesos en Firestore.
        </Text>
      </View>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          ...
          <Text variant="bodyMedium" style={{ marginTop: 12, opacity: 0.7 }}>Cargando administradores...</Text>
        </View>
      ) : (
        <>
          {admins.length > 0 && (
                      ...
                      <Card.Content style={styles.cardContent}>
                        ...
                        <View style={styles.cardInfo}>
                          ...
                          <Text variant="bodySmall" style={styles.descriptionText} numberOfLines={2}>
                            {item.description || 'Administrador general del negocio.'}
                          </Text>
            ...
            <Dialog.Content style={{ paddingBottom: 0 }}>
              <ScrollView style={{ maxHeight: 350 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={true}>
                ...
                <TextInput
                  ...
                  numberOfLines={2}
                  style={styles.input}
                  placeholder="Administrador de sucursal, soporte técnico..."
                  disabled={loadingSave}
                />
      ...
      <Portal>
        ...
        >
          <Dialog.Title style={{ color: theme.colors.error }}>¿Eliminar administrador?</Dialog.Title>
          <Dialog.Content>
            <Text variant="bodyMedium">
              ¿Estás seguro de que deseas eliminar permanentemente a <Text style={{ fontWeight: 'bold' }}>"{selectedAdmin?.name}"</Text> del sistema?
            </Text>
          </Dialog.Content>
          <Dialog.Actions>
