...
import { Avatar, Card, Divider, IconButton, List, Text, useTheme, Button } from 'react-native-paper';
import { SafeAreaView } from 'react-native-safe-area-context';

export default function CustomerUserDetailScreen() {
  const theme = useTheme();
  const router = useRouter();
  const params = useLocalSearchParams();

  // Extract customer details passed from query params
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <IconButton icon="arrow-left" size={24} onPress={() => router.push('/(admin)/customers')} style={{ marginLeft: -12, marginRight: 4 }} />
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileHeader}>
          ...
          <Text variant="bodyLarge" style={[styles.roleText, { color: isActive === 'false' ? theme.colors.error : theme.colors.outline }]}>
            Cliente Registrado {isActive === 'false' ? '• Acceso Inactivo' : '• Acceso Activo'}
          </Text>
        </View>
        ...
        <Card style={[styles.card, { backgroundColor: theme.colors.surface, marginTop: 16 }]} elevation={2}>
          <Card.Title
            ...
            left={(props) => <List.Icon {...props} icon="content-cut" color={theme.colors.primary} />}
          />
          <Card.Content>
            <Text variant="bodyMedium" style={{ lineHeight: 22, opacity: 0.8 }}>
              {description || 'Sin especificación de corte o preferencias registradas.'}
            </Text>
          </Card.Content>
        </Card>
